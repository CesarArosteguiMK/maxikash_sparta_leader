...
if not exist "%PYTHON_EXE%" (
    ...
    if not errorlevel 1 (
        ...
        if not errorlevel 1 (
            ...
            echo         Ejecute instalar-agente.bat o instale Python 3 en PATH.
            pause
            exit /b 1
        )
    )
...
set "API_DIR=%API_DIR%"
powershell -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File "%API_DIR%\iniciar-agente.ps1"
if errorlevel 1 (
    echo [ERROR] No se pudo arrancar uvicorn. Revise dependencias o use instalar-agente.bat.
    pause
    exit /b 1
)

echo API verificacion documentos iniciada con %PY_SOURCE%: http://127.0.0.1:8000  ^(docs: /docs^)
echo Para detener: cerrar-agente.bat o Detener-API-Verificacion.bat
ping 127.0.0.1 -n 3 >nul
