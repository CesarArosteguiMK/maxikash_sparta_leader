...
def validacion_cruzada(
    ...
    if calidad_foto and calidad_foto != "ok":
        rechazado = True
        if "brillo" in calidad_foto:
            alertas.append("FOTO RECHAZADA: Exceso de brillo/luz. Vuelva a tomar la foto del documento.")
        if "borroso" in calidad_foto:
            alertas.append("FOTO RECHAZADA: Imagen borrosa/desenfocada. Vuelva a tomar la foto del documento.")
        if calidad_foto == "revisar_brillo_y_borroso":
            alertas = ["FOTO RECHAZADA: Imagen con brillo excesivo y borrosa. Vuelva a tomar la foto."]
