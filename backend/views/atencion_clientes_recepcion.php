...
<div class="container-fluid py-4">
    ...
    <div class="acr-header-gradient d-flex align-items-center gap-3 flex-wrap">
        <i class="fa-solid fa-flag-checkered fa-2x"></i>
        <div class="flex-grow-1">
            <h4 class="d-flex align-items-center gap-2 flex-wrap">
                5.- Recepción
                <span class="badge bg-white text-dark d-none" id="acr-badge-header" style="font-size:.72rem;"></span>
            </h4>
            <p class="mb-0">Operaciones en etapa <strong>Recepción</strong> del pipeline.</p>
        </div>
    </div>

    <div class="card shadow-sm">
        <div class="card-body p-3">
            <div id="acr-loader-main" class="text-center py-5 text-muted" style="display:block;">
                <div class="spinner-border spinner-border-sm me-2"></div>Cargando...
            </div>
            <div id="acr-lista-main"></div>
        </div>
    </div>
</div>
...
(function () {
    ...
    const ACR_URL = '/AtencionClientes/obtenerRecuperacionRecepcion';
    const ACR_VACIO = 'No hay operaciones en Recepción.';
    ...
    function acrRenderCard(item) {
        ...
        <div class="ac-card">
            ...
            <div class="ac-card-footer">
                <a class="acr-btn-pipeline" href="/MotosAdjudicadas/pipeline" target="_blank" rel="noopener">
    ...
    function acrSetBadgeHeader(n) {
        const el = document.getElementById('acr-badge-header');
        if (!el) return;
        if (n > 0) {
            el.textContent   = n;
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const loader = document.getElementById('acr-loader-main');
        const lista  = document.getElementById('acr-lista-main');
        if (!loader || !lista) return;

        fetch(ACR_URL, { headers: { 'Accept': 'application/json' }, credentials: 'same-origin' })
            ...
            .then(function (data) {
                ...
                acrSetBadgeHeader(n);
                if (n === 0) {
                    lista.innerHTML = acrSinDatos(ACR_VACIO);
                } else {
                    lista.innerHTML = data.datos.map(function (d) { return acrRenderCard(d); }).join('');
                }
            })
            .catch(function (err) {
