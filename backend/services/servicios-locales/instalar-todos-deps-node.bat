...
echo  ^(pip global por defecto; ver API\launcher\instalar-agente.bat /VENV^)
echo --------------------------------------------
echo [pip] Instalando setuptools ^(necesario para compilar opencv^)...
pip install setuptools --user --quiet
call "%BACKEND%\API\launcher\instalar-agente.bat" /SILENT
if errorlevel 1 (
