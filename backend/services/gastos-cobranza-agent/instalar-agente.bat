...
echo.
echo ============================================
echo   Gastos cobranza Agent - instalacion (npm)
echo   Carpeta: %AGENT_DIR%
echo   Puerto HTTP por defecto: 3120
echo ============================================
...
if not exist "%AGENT_DIR%\package.json" (
    echo [ERROR] No esta package.json aqui. Coloque instalar-agente.bat DENTRO de gastos-cobranza-agent.
    pause
    exit /b 1
)
...
echo      Opcional: cree .env junto a server.js ^(PORT, API_KEY, REPORTE_PYTHON, etc.^) si lo necesita.
echo      Para arrancar: iniciar-agente.bat
echo      ^(o doble clic en iniciar-agente-oculto.vbs para no ver la consola de arranque^)
echo.
pause
