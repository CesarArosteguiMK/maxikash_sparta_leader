...
/**
 * Tickets tipo Crédito problemático.
 */
class TicketCreditoProblematico extends Model
{
    public static function guardar(array $datos, $idPersonaCreador)
    {
        ...
        $tipo = isset($datos['tipo_solicitud']) ? trim((string) $datos['tipo_solicitud']) : '';
        ...
        if ($tipo === '') {
            return self::resultado(false, 'El tipo de solicitud es obligatorio.', null);
        }
        if (strlen($tipo) > 100) {
        ...
        try {
            $db = new Database();
            $db->CRUD(
                "INSERT INTO ticket_credito_problematico (id_persona_creador, tipo_solicitud, descripcion, nombre_archivo_original, ruta_adjunto, fecha_creacion) " .
                "VALUES (:id_persona_creador, :tipo_solicitud, :descripcion, :nombre_archivo_original, :ruta_adjunto, NOW())",
                [
            ...
            $id = $row ? (int) $row['id'] : 0;
            return self::resultado(true, 'Solicitud de crédito problemático registrada correctamente.', ['id' => $id]);
        } catch (\Exception $e) {
