...
/**
 * Tickets tipo Viáticos.
 */
class TicketViaticos extends Model
{
    public static function guardar(array $datos, $idPersonaCreador)
    {
        ...
        $tipo = isset($datos['tipo_viatico']) ? trim((string) $datos['tipo_viatico']) : '';
        ...
        if ($tipo === '') {
            return self::resultado(false, 'El tipo de viático es obligatorio.', null);
        }
        if (strlen($tipo) > 100) {
        ...
        try {
            $db = new Database();
            $db->CRUD(
                "INSERT INTO ticket_viaticos (id_persona_creador, tipo_viatico, descripcion, nombre_archivo_original, ruta_adjunto, fecha_creacion) " .
                "VALUES (:id_persona_creador, :tipo_viatico, :descripcion, :nombre_archivo_original, :ruta_adjunto, NOW())",
                [
            ...
            $id = $row ? (int) $row['id'] : 0;
            return self::resultado(true, 'Solicitud de viáticos registrada correctamente.', ['id' => $id]);
        } catch (\Exception $e) {
