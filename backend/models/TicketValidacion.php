...
/**
 * Tickets tipo Validación (validar documento, dato o proceso).
 */
class TicketValidacion extends Model
{
    public static function guardar(array $datos, $idPersonaCreador)
    {
        ...
        $tipo = isset($datos['tipo_validacion']) ? trim((string) $datos['tipo_validacion']) : '';
        ...
        if ($tipo === '') {
            return self::resultado(false, 'El tipo de validación es obligatorio.', null);
        }
        if (strlen($tipo) > 100) {
        ...
        try {
            $db = new Database();
            $db->CRUD(
                "INSERT INTO ticket_validacion (id_persona_creador, tipo_validacion, descripcion, resultado, nombre_archivo_original, ruta_adjunto, fecha_creacion) " .
                "VALUES (:id_persona_creador, :tipo_validacion, :descripcion, 'pendiente', :nombre_archivo_original, :ruta_adjunto, NOW())",
                [
            ...
            $id = $row ? (int) $row['id'] : 0;
            return self::resultado(true, 'Solicitud de validación registrada correctamente.', ['id' => $id]);
        } catch (\Exception $e) {
